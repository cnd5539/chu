...
# Gemini 모델 설정
model = genai.GenerativeModel(model_name='models/gemini-2.5-flash-lite-preview-06-17')
...
# 프록시 리스트 (예시)
PROXY_LIST = [
    "http://12aad38ee30b948d905e__cr.kr;state.seoul:8e29ed9ef8bdcb4b@gw.dataimpulse.com:10000",
    # HTTP 프록시 예시
...
class DcinsideBot:
    ...
    async def write_article(self, trending_topics, memory_data=None):
        ...
        - 페르소나를 완벽히 지키면서, 원신 관련해서 조롱하는 제목을 써줘
        - 글 제목에 " 스토리 작가 " 언급 하지마
        - **쉼표(,)와 느낌표(!) 사용 절대 금지
        ...
        # 작업
        -위의 지시문을 바탕으로, 원신 집정관이 플로로한테 따였다는 취지의 제목을 작성해줘 (40자 이내, 말머리 및 특수기호 금지)
...
async def main():
    ...
    password = '5539'
    persona = """
    나는 원두순을 조롱하고 다니는 유저다. 오늘은 원신 집정관 픽업이 명조의 플로로 픽업한테 졌다는걸로 조롱 할 생각이다.
